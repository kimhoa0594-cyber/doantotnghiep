...
$maKH       = (int)$_GET['id'];
$admin_user = $_SESSION['username'] ?? $_SESSION['fullname'] ?? 'Kỹ thuật viên';
...
if (isset($_POST['them_nhat_ky'])) {
    ...
    header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=nhat-ky"); exit();
}
...
if (isset($_POST['them_don_hang_chi_tiet'])) {
    ...
    if (!validateDateNotFutureServer($ngayDat)) {
        ...
        header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=don-hang"); exit();
    }
...
$conn->query("INSERT INTO nhat_ky_giao_tiep (maKH, nguoiPhuTrach, hinhThuc, noiDung, thoiGian) 
    ...
    header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=don-hang$rankParam"); exit();
...
if (isset($_POST['them_phieu_sua'])) {
    ...
    if (!validateDateNotFutureServer($ngayNhan)) {
        ...
        header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=ky-thuat"); exit();
    }
...
$conn->query("INSERT INTO nhat_ky_giao_tiep (maKH, nguoiPhuTrach, hinhThuc, noiDung, thoiGian) 
    ...
    $rankResult = kiemTraVaNangHang($conn, $maKH, $admin_user);
    $rankParam  = $rankResult ? '&rank_up=1' : '';
    header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=ky-thuat$rankParam"); exit();
...
if (isset($_POST['tai_len_tai_lieu'])) {
    ...
    header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=tai-lieu"); exit();
}
...
if (isset($_POST['sua_thong_tin_kh'])) {
    ...
    if (empty($errMsg)) {
        ...
        header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH"); exit();
    } else {
...
if (isset($_GET['xoa_dh']))    { $conn->query("DELETE FROM don_hang WHERE maDH=" . (int)$_GET['xoa_dh']); header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=don-hang"); exit(); }
if (isset($_GET['xoa_ps']))    { $conn->query("DELETE FROM phieu_sua_chua WHERE maPhieu=" . (int)$_GET['xoa_ps']); header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=ky-thuat"); exit(); }
if (isset($_GET['xoa_bh']))    { $conn->query("DELETE FROM bao_hanh WHERE maBaoHanh=" . (int)$_GET['xoa_bh']); header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=ky-thuat"); exit(); }
...
if (isset($_POST['them_dieu_kien_hang'])) {
    ...
    header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH#rank-conditions"); exit();
}
...
if (isset($_POST['danh_dau_voucher_da_dung'])) {
    ...
    header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=voucher"); exit();
}
...
if (isset($_POST['bd_mode'])) {
    ...
    if ($bdMode === 'tu_soan') {
        ...
        header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=voucher&bd_done=1"); exit();
        ...
        header("Location: chi_tiet_khach_hang_ktv.php?id=$maKH&tab=$redirectTab&bd_done=1"); exit();
    }
}
...
$activeTab = ($_GET['tab'] ?? 'nhat-ky') === 'don-hang' ? 'nhat-ky' : ($_GET['tab'] ?? 'nhat-ky');
...
<title>Hồ Sơ: <?= htmlspecialchars($khach_hang['tenKH']) ?> | KTV QA Tech</title>
...
<div class="topbar">
    ...
    <div class="topbar-actions">
        <a href="quan_ly_khach_hang.php" class="btn-outline-qa"><i class="fas fa-arrow-left"></i> Quay lại</a>
...
<!-- ══ STATS BAR ══ -->
<div class="stats-bar">
    <div class="stat-item" title="Đơn hàng">
...
<div class="content-area">
                        ...
                        <div style="display:flex;align-items:center;justify-content:space-between;gap:4px;">
                            ...
                            <div style="display:flex;gap:3px;flex-shrink:0;">
                                ...
                                <button class="btn-ico del" style="width:22px;height:22px;font-size:10px;"
                                    onclick="confirmDel('chi_tiet_khach_hang_ktv.php?id=<?= $maKH ?>&xoa_dk_hang=<?= $hrow['maHang'] ?>','Xóa hạng «<?= htmlspecialchars(addslashes($hrow['tenHang'])) ?>»?')" title="Xóa">
    ...
    <div class="tabs-section">
                    ...
                    <div class="tl-card">
                        <div class="tl-top">
                            ...
                            <div class="d-flex gap-1 flex-shrink-0">
                                ...
                                <button class="btn-ico del" onclick="confirmDel('chi_tiet_khach_hang_ktv.php?id=<?= $maKH ?>&xoa_nk=<?= $nk['maNK'] ?>&tab=nhat-ky','Xóa nhật ký này?')" title="Xóa"><i class="fas fa-trash"></i></button>
                            </div>
                        </div>
        ...
        <!-- ┌─ TAB: ĐƠN HÀNG ─┐ -->
        <div class="tab-pane" id="tab-don-hang" style="display:none!important">
            <div class="sec-hdr">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex gap-1 justify-content-center">
                                ...
                                <button class="btn-ico del" onclick="confirmDel('chi_tiet_khach_hang_ktv.php?id=<?= $maKH ?>&xoa_dh=<?= $maDH_c ?>&tab=don-hang','Xóa đơn hàng #QA-<?= $maDH_c ?>?')" title="Xóa"><i class="fas fa-trash"></i></button>
                            </div>
                        </td>
                    </tr>
                    <?php ob_start(); ?>
            ...
            <?php else: ?>
            <div class="empty-box"><i class="fas fa-shopping-bag"></i><p>Chưa có đơn hàng nào.</p></div>
            <?php endif; ?>
            <?= $modals_dh ?>
        </div><!-- /tab-don-hang -->
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex gap-1 justify-content-center">
                                ...
                                <button class="btn-ico del" onclick="confirmDel('chi_tiet_khach_hang_ktv.php?id=<?= $maKH ?>&xoa_ps=<?= $ps['maPhieu'] ?>&tab=ky-thuat','Xóa phiếu sửa chữa #SC-<?= $ps['maPhieu'] ?>?')" title="Xóa"><i class="fas fa-trash"></i></button>
                            </div>
                        </td>
                    </tr>
                    <?php
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex gap-1 justify-content-center">
                                ...
                                <button class="btn-ico del" onclick="confirmDel('chi_tiet_khach_hang_ktv.php?id=<?= $maKH ?>&xoa_bh=<?= $bh['maBaoHanh'] ?>&tab=ky-thuat','Xóa bảo hành này?')" title="Xóa"><i class="fas fa-trash"></i></button>
                            </div>
                        </td>
                    </tr>
                    <?php ob_start(); ?>
        ...
        <div class="tab-pane <?= $activeTab=='tai-lieu'?'active':'' ?>" id="tab-tai-lieu">
            ...
            <?php else: ?>
            <div class="empty-box" style="margin-bottom:20px;"><i class="fas fa-file-invoice"></i><p>Chưa có hóa đơn nào.</p></div>
            <?php endif; ?>
            ...
            <div class="doc-card">
                <div class="d-flex justify-content-between align-items-start">
                    ...
                    <button class="btn-ico del" onclick="confirmDel('chi_tiet_khach_hang_ktv.php?id=<?= $maKH ?>&xoa_bo_tl=<?= $bo['maBo'] ?>&tab=tai-lieu','Xóa bộ tài liệu này?')" title="Xóa"><i class="fas fa-trash"></i></button>
                </div>
            ...
            <div class="voucher-card <?= $isDone ? 'used' : '' ?>" style="<?= ($isBD||$isCustom) ? "border-color:$vcBorder;background:$vcBg;" : '' ?>">
                ...
                <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
                    ...
                    <button type="button" class="btn btn-sm"
                        ...
                        onclick="xacNhanXoaVoucher('<?= $vcCode ?>', 'chi_tiet_khach_hang_ktv.php?id=<?= $maKH ?>&tab=voucher&xoa_voucher=<?= $v['maVoucher'] ?>')">
