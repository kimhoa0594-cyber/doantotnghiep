...
// ========================
$adminNotifsForThisKH = [];
$notifRes = $conn->query("
    SELECT tb.*, ps.tenThietBi, ps.tenKH, ps.trangThai as ttPhieu, ps.maPhieu
    FROM thong_bao_admin tb
    LEFT JOIN phieu_sua_chua ps ON ps.maPhieu = tb.maPhieu
    WHERE tb.trangThai = 'chua_doc' AND ps.maKH = $maKH
    ORDER BY tb.thoiGian DESC
    LIMIT 10
");
if ($notifRes) {
    while ($row = $notifRes->fetch_assoc()) {
        $adminNotifsForThisKH[] = $row;
    }
}
...
// Ưu tiên 1: Lấy từ bảng ky_thuat_vien (trangThai = 1 là đang hoạt động)
$ktvTableCheck = $conn->query("SHOW TABLES LIKE 'ky_thuat_vien'");
if ($ktvTableCheck && $ktvTableCheck->num_rows > 0) {
    $ktvRes = $conn->query("SELECT tenKTV AS username, tenKTV AS tenHienThi FROM ky_thuat_vien WHERE trangThai = 1 ORDER BY tenKTV ASC");
    if ($ktvRes) { while ($t = $ktvRes->fetch_assoc()) $dsTech[] = $t; }
}

// Ưu tiên 2: Bổ sung từ bảng users (role = 'technician', status = 1) nếu chưa có trong danh sách trên
if (empty($dsTech)) {
    $usersTableCheck = $conn->query("SHOW TABLES LIKE 'users'");
    if ($usersTableCheck && $usersTableCheck->num_rows > 0) {
        $usersRes = $conn->query("SELECT COALESCE(fullname, username) AS username, COALESCE(fullname, username) AS tenHienThi FROM users WHERE role = 'technician' AND status = 1 ORDER BY COALESCE(fullname, username) ASC");
        if ($usersRes) {
            $existing = array_column($dsTech, 'tenHienThi');
            while ($t = $usersRes->fetch_assoc()) {
                if (!in_array($t['tenHienThi'], $existing)) {
                    $dsTech[] = $t;
                    $existing[] = $t['tenHienThi'];
                }
            }
        }
    }
...
// Ưu tiên 3: Lấy từ bảng tai_khoan nếu vẫn rỗng (dự phòng tương thích cũ)
if (empty($dsTech)) {
    $dsTechResult = $conn->query("SHOW TABLES LIKE 'tai_khoan'");
    if ($dsTechResult && $dsTechResult->num_rows > 0) {
        $colCheck = $conn->query("SHOW COLUMNS FROM tai_khoan");
        $cols = [];
        if ($colCheck) { while ($c = $colCheck->fetch_assoc()) $cols[] = $c['Field']; }
        $hasRole = in_array('role', $cols);
        if ($hasRole) {
            $nameCol = in_array('tenHienThi', $cols) ? 'tenHienThi' : (in_array('hoTen', $cols) ? 'hoTen' : 'username');
            $hasStatus = in_array('trangThai', $cols);
            $whereStatus = $hasStatus ? " AND (trangThai = 1 OR trangThai = 'active')" : '';
            $techRes = $conn->query("SELECT username, $nameCol AS tenHienThi FROM tai_khoan WHERE role = 'kythuatvien'$whereStatus ORDER BY $nameCol ASC");
            if ($techRes) { while ($t = $techRes->fetch_assoc()) $dsTech[] = $t; }
        }
    }
}

// Ưu tiên 4: Lấy từ bảng nhan_vien nếu vẫn rỗng
if (empty($dsTech)) {
    $nvCheck = $conn->query("SHOW TABLES LIKE 'nhan_vien'");
    if ($nvCheck && $nvCheck->num_rows > 0) {
        $nvRes = $conn->query("SELECT tenNV AS username, tenNV AS tenHienThi FROM nhan_vien WHERE chucVu LIKE '%kỹ thuật%' OR chucVu LIKE '%ky thuat%' ORDER BY tenNV ASC");
        if ($nvRes) { while ($t = $nvRes->fetch_assoc()) $dsTech[] = $t; }
    }
}


// Lấy số phiếu đang xử lý (chưa hoàn thành) của từng kỹ thuật viên
$ktvWorkload = [];
if (!empty($dsTech)) {
    $wlResult = $conn->query("
        SELECT kyThuatVien, COUNT(*) as so_phieu
        FROM phieu_sua_chua
        WHERE trangThai NOT IN ('Đã bàn giao', 'Đã sửa xong')
          AND kyThuatVien IS NOT NULL AND kyThuatVien != ''
        GROUP BY kyThuatVien
    ");
    if ($wlResult) {
        while ($wl = $wlResult->fetch_assoc()) {
            $ktvWorkload[$wl['kyThuatVien']] = (int)$wl['so_phieu'];
        }
    }
}
=======
...
<<<<<<< HEAD
    ...
    // === GỬI THÔNG BÁO CHO KỸ THUẬT VIÊN NẾU ĐƯỢC CHỈ ĐỊNH ===
    if (!empty($kyThuatVien) && $maPhieuMoi) {
        // Lấy thông tin chi tiết khách hàng để gửi thông báo
        $khInfo = $khach_hang;
        
        $tieuDe = "🔧 Bạn được phân công sửa chữa #SC-$maPhieuMoi";
        $noiDung = "Bạn được admin phân công xử lý phiếu sửa chữa:
━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 Mã phiếu: #SC-$maPhieuMoi
👤 Khách hàng: {$khInfo['tenKH']}
📞 SĐT: {$khInfo['soDienThoai']}
💻 Thiết bị: $tenThietBi
📝 Mô tả: " . mb_substr($moTaLoiEsc, 0, 150) . (mb_strlen($moTaLoiEsc) > 150 ? '...' : '')
 . "
━━━━━━━━━━━━━━━━━━━━━━━━━━
👉 Vui lòng truy cập phiếu sửa chữa và cập nhật trạng thái theo tiến độ.
⏰ Ngày phân công: " . date('d/m/Y H:i');
        
        // Thêm vào bảng thông báo
        $stmtNotif = $conn->prepare("INSERT INTO thong_bao_ktv (maPhieu, kyThuatVien, loai, tieuDe, noiDung, nguoiGui, trangThai) 
                                     VALUES (?, ?, 'phan_cong', ?, ?, ?, 'chua_doc')");
        $stmtNotif->bind_param("issss", $maPhieuMoi, $kyThuatVien, $tieuDe, $noiDung, $admin_user);
        $stmtNotif->execute();
        $stmtNotif->close();
    }
...
<<<<<<< HEAD
    ...
    $oldStmt = $conn->prepare("SELECT kyThuatVien, tenThietBi, moTaLoi FROM phieu_sua_chua WHERE maPhieu = ?");
    $oldStmt->bind_param("i", $maPhieu);
    $oldStmt->execute();
    $oldPhieu = $oldStmt->get_result()->fetch_assoc();
    $oldKTV = $oldPhieu['kyThuatVien'] ?? '';
    $oldTenThietBi = $oldPhieu['tenThietBi'] ?? '';
    $oldMoTa = $oldPhieu['moTaLoi'] ?? '';
    $oldStmt->close();
    
    $kyThuatVienSQL = !empty($kyThuatVien) ? "'$kyThuatVien'" : 'NULL';
    $conn->query("UPDATE phieu_sua_chua SET tenThietBi='$tenThietBi', moTaLoi='$moTaLoi', ngayNhan='$ngayNhan', ngayTra=$ngayTra, chiPhi=$chiPhi, trangThai='$trangThai', kyThuatVien=$kyThuatVienSQL WHERE maPhieu=$maPhieu");
    
    // === GỬI THÔNG BÁO CHO KỸ THUẬT VIÊN MỚI NẾU ĐƯỢC CHỈ ĐỊNH HOẶC THAY ĐỔI ===
    if (!empty($kyThuatVien) && $kyThuatVien != $oldKTV) {
        // Lấy thông tin khách hàng
        $khInfo = $khach_hang;
        
        $tieuDe = "🔧 Bạn được phân công sửa chữa #SC-$maPhieu";
        $noiDung = "Bạn được admin phân công xử lý phiếu sửa chữa:
━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 Mã phiếu: #SC-$maPhieu
👤 Khách hàng: {$khInfo['tenKH']}
📞 SĐT: {$khInfo['soDienThoai']}
💻 Thiết bị: $tenThietBi
📝 Mô tả: " . mb_substr($moTaLoi, 0, 150) . (mb_strlen($moTaLoi) > 150 ? '...' : '')
 . "
━━━━━━━━━━━━━━━━━━━━━━━━━━
👉 Vui lòng truy cập phiếu sửa chữa và cập nhật trạng thái theo tiến độ.
⏰ Ngày phân công: " . date('d/m/Y H:i');
        
        // Kiểm tra xem đã có thông báo chưa đọc cho phiếu này chưa (để tránh trùng)
        $checkStmt = $conn->prepare("SELECT id FROM thong_bao_ktv WHERE maPhieu = ? AND kyThuatVien = ? AND trangThai = 'chua_doc' LIMIT 1");
        $checkStmt->bind_param("is", $maPhieu, $kyThuatVien);
        $checkStmt->execute();
        ...
        if (!$exists) {
            ...
            $stmtNotif = $conn->prepare("INSERT INTO thong_bao_ktv (maPhieu, kyThuatVien, loai, tieuDe, noiDung, nguoiGui, trangThai) 
                                         VALUES (?, ?, 'phan_cong', ?, ?, ?, 'chua_doc')");
            $stmtNotif->bind_param("issss", $maPhieu, $kyThuatVien, $tieuDe, $noiDung, $admin_user);
            $stmtNotif->execute();
            $stmtNotif->close();
        }
    }
                        ...
                        <td>
                            <?php if (!empty($ps['kyThuatVien'])): ?>
                                <div style="display:flex;align-items:center;gap:6px;">
                                    <div style="width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,#3b82f6,#2563eb);display:flex;align-items:center;justify-content:center;color:#fff;font-size:11px;font-weight:700;flex-shrink:0;">
                                        <?= mb_strtoupper(mb_substr($ps['kyThuatVien'], 0, 1)) ?>
                                    </div>
                                    <div>
                                        <div style="font-size:12px;font-weight:600;color:#1e293b;"><?= htmlspecialchars($ps['kyThuatVien']) ?></div>
                                        <div style="font-size:10px;color:#64748b;">Kỹ thuật viên</div>
                                    </div>
                                </div>
                            <?php else: ?>
                                <button class="btn btn-sm" style="font-size:11px;background:#fef3c7;border:1px solid #fbbf24;color:#92400e;border-radius:8px;padding:3px 10px;cursor:pointer;" data-bs-toggle="modal" data-bs-target="#edtPS_<?= $ps['maPhieu'] ?>">
                                    <i class="fas fa-user-plus me-1"></i>Chỉ định KTV
                                </button>
                            <?php endif; ?>
                        </td>
=======
...
<<<<<<< HEAD
                                                ...
                                                <div style="display:flex;justify-content:space-between;align-items:center;">
                                                    ...
                                                    <span style="display:inline-flex;align-items:center;gap:5px;">
                                                        <span style="width:20px;height:20px;background:linear-gradient(135deg,#3b82f6,#2563eb);border-radius:50%;display:inline-flex;align-items:center;justify-content:center;color:#fff;font-size:9px;font-weight:800;flex-shrink:0;"><?= mb_strtoupper(mb_substr($ps['kyThuatVien'],0,1,'UTF-8'),'UTF-8') ?></span>
                                                        <strong style="color:#1e40af;"><?= htmlspecialchars($ps['kyThuatVien']) ?></strong>
                                                    </span>
                                                    <?php else: ?>
...
<<<<<<< HEAD
                                            ...
                                            <div class="col-12">
                                                <label style="font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#94a3b8;display:flex;align-items:center;gap:6px;margin-bottom:10px;">
                                                    <i class="fas fa-user-cog"></i> Phân công kỹ thuật viên
                                                    <span style="flex:1;height:1px;background:#e2e8f0;"></span>
                                                </label>
                                                <?php if (!empty($dsTech)): ?>
                                                <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;" id="ktvPickerEdit_<?= $ps['maPhieu'] ?>">
                                                    <!-- Không chỉ định -->
                                                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:8px 10px;border:2px solid <?= empty($ps['kyThuatVien']) ? '#3b82f6' : '#e2e8f0' ?>;border-radius:9px;background:<?= empty($ps['kyThuatVien']) ? '#eff6ff' : '#fff' ?>;transition:all .18s;grid-column:1/-1;" class="ktv-card-edit-<?= $ps['maPhieu'] ?>">
                                                        <input type="radio" name="kyThuatVien" value="" <?= empty($ps['kyThuatVien']) ? 'checked' : '' ?> style="display:none;" class="ktv-radio-edit-<?= $ps['maPhieu'] ?>">
                                                        <div style="width:30px;height:30px;background:#f1f5f9;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;">👤</div>
                                                        <div style="flex:1;">
                                                            <div style="font-size:12px;font-weight:700;color:#64748b;">Chưa chỉ định</div>
                                                            <div style="font-size:10px;color:#94a3b8;">Gỡ bỏ phân công hiện tại</div>
                                                        </div>
                                                    </label>
                                                    <?php foreach ($dsTech as $tech):
                                                        $ten = htmlspecialchars($tech['tenHienThi'] ?? $tech['username']);
                                                        $soPhieu = $ktvWorkload[$tech['tenHienThi'] ?? $tech['username']] ?? 0;
                                                        $isAssigned = ($ps['kyThuatVien'] === ($tech['tenHienThi'] ?? $tech['username']));
                                                        // Nếu KTV đang phụ trách phiếu này thì trừ đi 1 để hiện đúng
                                                        $displayPhieu = $isAssigned ? max(0, $soPhieu - 1) : $soPhieu;
                                                        if ($displayPhieu == 0) {
                                                            $sc = '#059669'; $sbg = '#d1fae5'; $sbd = '#6ee7b7';
                                                            $st = 'Rảnh'; $si2 = '🟢';
                                                        } elseif ($displayPhieu <= 2) {
                                                            $sc = '#d97706'; $sbg = '#fef3c7'; $sbd = '#fde68a';
                                                            $st = "$displayPhieu phiếu"; $si2 = '🟡';
                                                        } else {
                                                            $sc = '#dc2626'; $sbg = '#fee2e2'; $sbd = '#fca5a5';
                                                            $st = "Bận ($displayPhieu)"; $si2 = '🔴';
                                                        }
                                                        $init = mb_strtoupper(mb_substr($ten, 0, 1, 'UTF-8'), 'UTF-8');
                                                        $bdrColor = $isAssigned ? '#3b82f6' : '#e2e8f0';
                                                        $bgColor  = $isAssigned ? '#eff6ff' : '#fff';
                                                    ?>
                                                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:8px 10px;border:2px solid <?= $bdrColor ?>;border-radius:9px;background:<?= $bgColor ?>;transition:all .18s;" class="ktv-card-edit-<?= $ps['maPhieu'] ?>">
                                                        <input type="radio" name="kyThuatVien" value="<?= $ten ?>" <?= $isAssigned ? 'checked' : '' ?> style="display:none;" class="ktv-radio-edit-<?= $ps['maPhieu'] ?>">
                                                        <div style="width:30px;height:30px;background:linear-gradient(135deg,#3b82f6,#2563eb);border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:800;flex-shrink:0;"><?= $init ?></div>
                                                        <div style="flex:1;min-width:0;">
                                                            <div style="font-size:12px;font-weight:700;color:#0f172a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= $ten ?></div>
                                                            <span style="font-size:10px;font-weight:700;color:<?= $sc ?>;background:<?= $sbg ?>;border:1px solid <?= $sbd ?>;border-radius:20px;padding:1px 6px;"><?= $si2 ?> <?= $st ?></span>
                                                        </div>
                                                    </label>
                                                    <?php endforeach; ?>
                                                </div>
                                                <script>
                                                (function(){
                                                    var radios = document.querySelectorAll('.ktv-radio-edit-<?= $ps['maPhieu'] ?>');
                                                    radios.forEach(function(r){
                                                        r.addEventListener('change',function(){
                                                            document.querySelectorAll('.ktv-card-edit-<?= $ps['maPhieu'] ?>').forEach(function(l){
                                                ...
                                                <?php else: ?>
                                                <div style="background:#f8fafc;border:1px dashed #cbd5e1;border-radius:9px;padding:12px;text-align:center;color:#94a3b8;font-size:12px;">
                                                    Chưa có kỹ thuật viên. <input type="hidden" name="kyThuatVien" value="<?= htmlspecialchars($ps['kyThuatVien'] ?? '') ?>">
                                                </div>
                                                <?php endif; ?>
                                            </div>
=======
                                        ...
                                        <tr>
                                            ...
                                            <td style="padding:9px 12px;border-bottom:1px solid #e2e8f0;">
                                                ...
                                                <span style="display:inline-flex;align-items:center;gap:6px;">
                                                    <span style="width:22px;height:22px;background:linear-gradient(135deg,#3b82f6,#2563eb);border-radius:50%;display:inline-flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:800;"><?= mb_strtoupper(mb_substr($ps['kyThuatVien'],0,1,'UTF-8'),'UTF-8') ?></span>
                                                    <strong style="color:#1e40af;"><?= htmlspecialchars($ps['kyThuatVien']) ?></strong>
                                                </span>
                                                <?php else: ?>
...
<<<<<<< HEAD
                                            ...
                                            <div style="font-size:11px;font-weight:600;color:#1e40af;margin-top:2px;">
                                                <?php if (!empty($ps['kyThuatVien'])): ?>
                                                    <?= htmlspecialchars($ps['kyThuatVien']) ?>
                                                <?php else: ?>
...
<<<<<<< HEAD
                            ...
                            <?php if (!empty($dsTech)): ?>
                            <div style="display:flex;flex-direction:column;gap:6px;" id="ktvPickerCreate">
                                <!-- Không chỉ định -->
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:2px solid #e2e8f0;border-radius:10px;background:#fff;transition:all .18s;" class="ktv-card-create">
                                    <input type="radio" name="kyThuatVien" value="" checked style="display:none;" class="ktv-radio-create">
                                    <div style="width:36px;height:36px;background:#f1f5f9;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0;">👤</div>
                                    <div style="flex:1;">
                                        <div style="font-size:13px;font-weight:700;color:#64748b;">Chưa chỉ định</div>
                                        <div style="font-size:11px;color:#94a3b8;">Admin sẽ phân công sau</div>
                                    </div>
                                    <div class="ktv-radio-dot" style="width:18px;height:18px;border-radius:50%;border:2px solid #cbd5e1;flex-shrink:0;display:flex;align-items:center;justify-content:center;transition:all .15s;"></div>
                                </label>
                                <?php foreach ($dsTech as $tech):
                                    $ten = htmlspecialchars($tech['tenHienThi'] ?? $tech['username']);
                                    $un  = htmlspecialchars($tech['username']);
                                    $soPhieu = $ktvWorkload[$tech['tenHienThi'] ?? $tech['username']] ?? 0;
                                    if ($soPhieu == 0) {
                                        $statusColor = '#059669'; $statusBg = '#d1fae5'; $statusBorder = '#6ee7b7';
                                        $statusText  = 'Rảnh'; $statusIcon = '🟢';
                                    } elseif ($soPhieu <= 2) {
                                        $statusColor = '#d97706'; $statusBg = '#fef3c7'; $statusBorder = '#fde68a';
                                        $statusText  = "$soPhieu phiếu"; $statusIcon = '🟡';
                                    } else {
                                        $statusColor = '#dc2626'; $statusBg = '#fee2e2'; $statusBorder = '#fca5a5';
                                        $statusText  = "Bận ($soPhieu phiếu)"; $statusIcon = '🔴';
                                    }
                                    $initial = mb_strtoupper(mb_substr($ten, 0, 1, 'UTF-8'), 'UTF-8');
                                ?>
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:2px solid #e2e8f0;border-radius:10px;background:#fff;transition:all .18s;" class="ktv-card-create" data-busy="<?= $soPhieu ?>">
                                    <input type="radio" name="kyThuatVien" value="<?= $ten ?>" style="display:none;" class="ktv-radio-create">
                                    <div style="width:36px;height:36px;background:linear-gradient(135deg,#3b82f6,#2563eb);border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-size:14px;font-weight:800;flex-shrink:0;"><?= $initial ?></div>
                                    <div style="flex:1;min-width:0;">
                                        <div style="font-size:13px;font-weight:700;color:#0f172a;"><?= $ten ?></div>
                                        <div style="font-size:11px;color:#64748b;">Kỹ thuật viên</div>
                                    </div>
                                    <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;">
                                        <span style="font-size:10px;font-weight:700;color:<?= $statusColor ?>;background:<?= $statusBg ?>;border:1px solid <?= $statusBorder ?>;border-radius:20px;padding:2px 8px;white-space:nowrap;"><?= $statusIcon ?> <?= $statusText ?></span>
                                        <?php if ($soPhieu > 0): ?>
                                        <span style="font-size:10px;color:#94a3b8;"><?= $soPhieu ?> phiếu đang xử lý</span>
                                        <?php endif; ?>
                                    </div>
                                    <div class="ktv-radio-dot" style="width:18px;height:18px;border-radius:50%;border:2px solid #cbd5e1;flex-shrink:0;display:flex;align-items:center;justify-content:center;transition:all .15s;"></div>
                                </label>
                            ...
                            <?php else: ?>
                            <div style="background:#f8fafc;border:1px dashed #cbd5e1;border-radius:10px;padding:14px;text-align:center;color:#94a3b8;font-size:12px;">
                                <i class="fas fa-user-slash" style="font-size:20px;margin-bottom:6px;display:block;"></i>
                                Chưa có kỹ thuật viên nào trong hệ thống.<br>
                                <input type="hidden" name="kyThuatVien" value="">
                                <small>Vào phần Tài khoản để thêm kỹ thuật viên.</small>
                            </div>
                            <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function () {
    function updateKtvCreateUI() {
        document.querySelectorAll('.ktv-card-create').forEach(function (label) {
            var radio = label.querySelector('.ktv-radio-create');
            var dot   = label.querySelector('.ktv-radio-dot');
            if (!radio || !dot) return;
            ...
            if (radio.checked) {
                ...
                dot.innerHTML           = '<svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polyline points="1.5,5 4,7.5 8.5,2.5" stroke="#fff" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
            } else {
    ...
    /* Khởi tạo trạng thái ban đầu (card "Chưa chỉ định" mặc định checked) */
    updateKtvCreateUI();

    /* Lắng nghe click trên toàn bộ card */
    document.querySelectorAll('.ktv-card-create').forEach(function (label) {
        label.addEventListener('click', function () {
            /* radio bên trong tự được check bởi browser khi click label,
               nhưng ta gọi updateKtvCreateUI sau một tick để đảm bảo */
            setTimeout(updateKtvCreateUI, 0);
        });
    });
});
=======
